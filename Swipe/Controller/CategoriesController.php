...
class CategoriesController extends AppController 
{
	...
	public function admin_index() {
		if($this->request->is('post')){
            ...
            $this->set('categories',$this->Category->find('all',array('conditions'=>array("Category.name LIKE"=>"%$keyword%"),'order'=>'Category.id ASC')));
	}else{
		$this->Category->recursive = 0;
		$this->paginate = array('limit' =>10, 'order'=>'Category.id ASC');
		$this->set('categories', $this->paginate());
	}
	...
	public function admin_add() {
		if ($this->request->is('post')) {
				...
				if(empty($categoryExist)){
					...
					$this->request->data['Category']['image'] = $one['name'];
					$this->Category->create();
					if ($this->Category->save($this->request->data)) {	
						if ($one['error'] == 0) {
							$pth = 'files/categoryimages' . DS .$one['name'];
